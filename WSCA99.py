...
def process_differences(diff, original, updated):
    ...
    if 'dictionary_item_added' in diff:
        for added_item in diff['dictionary_item_added'].values():
            if isinstance(added_item, dict):  # Handle case where it is a dictionary
            ...
            elif isinstance(added_item, list):  # Handle case where it's a list
                for item in added_item:
                    id_value = create_unique_identifier(item)
                    if id_value:
                        output['added'].append({
                            'id': id_value,
                            'details': item
    ...
    if 'dictionary_item_removed' in diff:
        for removed_item in diff['dictionary_item_removed'].values():
            if isinstance(removed_item, dict):  # Handle case where it is a dictionary
            ...
            elif isinstance(removed_item, list):  # Handle case where it's a list
