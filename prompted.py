...
def extract_chunk(ix: int, chunk: str, prompt: str) -> Dict[str, Any]:
    print(f"[LOG] Processing chunk {ix}")
    
    # Construct the message using the prompt and the chunk
...
def run_extraction(html: str, prompt: str) -> List[Dict[str, Any]]:
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        futures = [executor.submit(extract_chunk, ix, chunk, prompt) for ix, chunk in enumerate(chunks)]
...
# Define the prompt
prompt = """
Extract the following information: names of individuals, associated URLs, addresses, entity type, and ID number. Make sure to include any other relevant data if available.
"""
