...
from ora import ora  # Import the ora model connection from ora.py
...
class CustomLLMExtractionStrategy(ExtractionStrategy):
    def __init__(self, provider: str = "org-model", api_token: str = None, 
        ...
        # Plan B: Setup for ora model
        if self.provider == "ora-model":
            self.ora_connection = ora(auth=os.getenv('ORA_API_KEY'), engine="llama2")
    ...
    def extract(self, url: str, ix: int, html: str) -> List[Dict[str, Any]]:
        """
        Extract content based on the provider (Plan A or Plan B).
        """
        print(f"[LOG] Processing bucket {ix} for URL: {url}")
        
        if self.provider == "org-model":
            # Plan A: Your organization's model using an API call
            payload = {
                "url": url,
                "html": html,
        ...
        elif self.provider == "ora-model":
            # Plan B: Using ora model
            input_message = html
            response = self.ora_connection.chat(msg=input_message)
            print(f"[LOG] Response from ora model: {response['reply']}")
